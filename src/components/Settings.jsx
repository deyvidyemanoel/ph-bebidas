    ...
    <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
      <div className="bg-dark-700 border border-dark-400 rounded-2xl w-full max-w-md">
        <div className="flex items-center gap-3 px-5 py-4 border-b border-dark-400">
          ...
          <h3 className="text-white font-bold text-lg">Importar dados</h3>
          <button onClick={onClose} className="ml-auto text-gray-500 hover:text-white transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          <p className="text-gray-300 text-sm leading-relaxed">
            O arquivo contém os seguintes dados que serão restaurados:
          </p>

          <div className="grid grid-cols-2 gap-2">
            {[
              { label: 'Produtos', value: data.products?.length ?? 0, color: 'text-blue-400' },
              { label: 'Vendas', value: data.sales?.length ?? 0, color: 'text-gold-400' },
              { label: 'Clientes', value: data.clients?.length ?? 0, color: 'text-purple-400' },
              { label: 'Movimentações', value: data.movements?.length ?? 0, color: 'text-emerald-400' },
            ].map(item => (
              <div key={item.label} className="bg-dark-600 rounded-xl p-3 text-center">
          ...
          {data.exportedAt && (
            <p className="text-gray-600 text-xs text-center">
              Backup gerado em: {new Date(data.exportedAt).toLocaleString('pt-BR')}
            </p>
          )}
...
export default function Settings({
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `ph-bebidas-backup-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  const handleFileSelect = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (!parsed.products || !parsed.sales || !parsed.clients || !parsed.movements) {
          showToast('Arquivo inválido: estrutura de dados não reconhecida.', 'error');
          return;
        }
      ...
      <div className="bg-dark-700 border border-dark-400 rounded-2xl p-5">
        ...
        <div className="flex items-start gap-2.5 bg-amber-500/8 border border-amber-500/20 rounded-xl p-3.5 mt-4">
          ...
          <p className="text-amber-200/70 text-xs leading-relaxed">
            Recomendamos exportar um backup periodicamente. Os dados existem apenas neste navegador e podem ser perdidos se o cache for limpo ou o dispositivo trocado.
          </p>
        </div>
        ...
        <div className="flex gap-3 mt-4">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 bg-dark-600 border border-dark-300 hover:border-gold-500/50 hover:bg-gold-500/8 text-gray-300 hover:text-gold-400 py-2.5 rounded-xl text-sm font-medium transition-colors"
          >
            <Download size={15} />
            Exportar dados
          </button>
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 bg-dark-600 border border-dark-300 hover:border-blue-500/50 hover:bg-blue-500/8 text-gray-300 hover:text-blue-400 py-2.5 rounded-xl text-sm font-medium transition-colors"
          >
            <Upload size={15} />
            Importar dados
          </button>
